...
const COPY: Record<'vi' | 'en', Copy> = {
  ...
  en: { eyebrow: 'YOUR FAVORITE APPS', tagline: 'One tidy home for all your tools.', apps: count => `${count} app${count === 1 ? '' : 's'}`, settings: 'Settings', editList: 'Edit app list', changeTheme: 'Change appearance', openInNewTab: 'Open in new tab', language: 'Language', save: 'Save', addApp: 'Add app', open: name => `Open ${name}`, edit: name => `Edit ${name}`, delete: name => `Delete ${name}`, confirmDelete: name => `Delete ${name}?`, editApp: 'Edit', addAppTitle: 'Add new', close: 'Close', loadingMetadata: 'Fetching title and favicon…', metadataHint: 'Leave the URL field to fill in the title and favicon.', invalidUrl: 'Enter a valid HTTP or HTTPS URL.', httpOnly: 'Only HTTP and HTTPS URLs are supported.', appName: 'App name', iconUrl: 'Icon URL', cancel: 'Cancel', saveChanges: 'Save changes', addToGrid: 'Add to grid', deleteApp: 'Delete app', themeDescription: 'Choose a background, display controls, and text color.', background: 'Background', default: 'Default', brightness: 'Brightness', blur: 'Blur', uploadImage: 'Upload image', textColor: 'Text color', custom: 'Custom' }
};
const CopyContext = createContext<Copy>(COPY.vi);
const useCopy = () => useContext(CopyContext);
const SYNC_COPY = {
  vi: { sync: 'Đồng bộ', export: 'Export', import: 'Import', clearAll: 'Xóa tất cả', title: 'Xóa tất cả app?', description: 'Thao tác này sẽ xóa toàn bộ app trong bản nháp. Cậu vẫn cần bấm Lưu để xác nhận.', confirm: 'Xóa tất cả', invalid: 'File JSON không hợp lệ.', success: (count: number) => `Đã import ${count} app.` },
  en: { sync: 'Sync', export: 'Export', import: 'Import', clearAll: 'Clear all', title: 'Clear all apps?', description: 'This removes every app from the draft. Press Save to commit the change.', confirm: 'Clear all', invalid: 'Invalid JSON file.', success: (count: number) => `Imported ${count} apps.` }
};
...
function SyncDialog({ bookmarks, text, onClose, onImport }: { bookmarks: Bookmark[]; text: (typeof SYNC_COPY)['vi']; onClose: () => void; onImport: (items: Bookmark[]) => void }) {
  const [error, setError] = useState('');
  const exportJson = () => { const blob = new Blob([JSON.stringify(bookmarks, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = 'app-shelf-bookmarks.json'; link.click(); URL.revokeObjectURL(url); };
  const importJson = async (event: React.ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (!file) return; try { const source = JSON.parse(await file.text()); if (!Array.isArray(source)) throw new Error(); const items = source.map((item): Bookmark => { if (!item || typeof item.url !== 'string' || typeof item.name !== 'string') throw new Error(); const url = new URL(item.url); if (!['http:', 'https:'].includes(url.protocol)) throw new Error(); return { id: typeof item.id === 'string' ? item.id : crypto.randomUUID(), name: item.name.trim() || hostname(url.href), url: url.href, iconUrl: typeof item.iconUrl === 'string' ? item.iconUrl : null, createdAt: typeof item.createdAt === 'string' ? item.createdAt : new Date().toISOString() }; }); onImport(items); onClose(); } catch { setError(text.invalid); } };
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}><section className="sync-popup" role="dialog" aria-label={text.sync} onMouseDown={event => event.stopPropagation()}><button type="button" onClick={exportJson}>⇩ {text.export}</button><label>⇧ {text.import}<input type="file" accept="application/json,.json" onChange={importJson} /></label>{error && <p>{error}</p>}</section></div>;
}
