...
export function saveBackground(background: string | null) {
  if (background) window.localStorage.setItem(BACKGROUND_KEY, background);
    ...
    if (saved) {
      ...
      if (typeof value?.brightness === 'number')
        return {
          ...
          textColor: typeof value.textColor === 'string' ? value.textColor : '#f8fafc',
        };
    }
  } catch {
    /* Use defaults below. */
...
export function saveBackgroundAppearance(appearance: BackgroundAppearance) {
  window.localStorage.setItem(BACKGROUND_APPEARANCE_KEY, JSON.stringify(appearance));
}
export function readOpenInNewTab() {
  return window.localStorage.getItem(OPEN_IN_NEW_TAB_KEY) !== 'false';
}
export function saveOpenInNewTab(value: boolean) {
  window.localStorage.setItem(OPEN_IN_NEW_TAB_KEY, String(value));
}
